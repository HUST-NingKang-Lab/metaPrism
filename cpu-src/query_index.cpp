...
int multi_query(string outpath,string filelist,string indexname,int n, int t, int group,string scroingtype,string filterflag, int is_index,Meta_Database &database){
...
int main(int argc, char * argv[]){
    ...
    cout<<"This query step costs: "<<time_use / 1000000 <<" s"<<endl;
          ...
          multi_query(outfilename,filelist,indexname,r_number,t_number,group,scroingtype,filterflag,is_index,database);
          endTime=clock();
          gettimeofday(&tv_end,NULL);
          double time_use=double(tv_end.tv_sec-tv_begin.tv_sec)*1000000+double(tv_end.tv_usec-tv_begin.tv_usec);
