...
unsigned int Meta_Database::Parallel_Indexed_Query_RAM(ostream & out, string infilename, string biotype,vector<string> &databaselist,map<string,float *> &database_map, int n, int t_number, int group,string scoringtype,string filterflag){
                        ...
                        for (int i = this->Key_dim; i>0; i--)
                            if (weight[i-1] >= q_min){
                                          ...
                                          for(int i=0 ; i < length ;i++ )
                                          {
                                            ...
                                            n_count += this->Parallel_Indexed_Query_RAM(infilename,key_set[i],biotype,database_map,results, n, t_number, group,scoringtype, filterflag,comparison_time);
                                          }
                                        }
